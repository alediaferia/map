...
perf_metrics_t run_map_test(perf_test_config_t *config, const char *output_file,
  ...
  case MAP_VALUE_SOURCE_CMDLINE_ARG:
    if (config->replacement_pattern && strlen(config->replacement_pattern)) {
      sprintf(cmd + strlen(cmd), "-v 'replacement: %s' ",
              config->replacement_pattern);
    } else {
  ...
  if (stat(output_file, &st) == 0) {
    ...
    metrics.output_size_alt =
        st.st_size; // Will be overwritten for alternative command
  } else {
...
perf_metrics_t run_alt_test(perf_test_config_t *config, const char *output_file,
  ...
  if (ret != 0) {
    fprintf(stderr,
            "Alternative command execution failed with return code %d\n", ret);
    // Don't exit, we'll still report the metrics
  }
...
perf_metrics_t run_performance_test(perf_test_config_t *config) {
  ...
  create_comparison_table(config->name, metrics_map, metrics_alt,
                          config->alternative_cmd);
  return metrics_map;
}
...
void create_comparison_table(const char *test_name, perf_metrics_t metrics_map,
  ...
  double throughput_diff =
      metrics_map.throughput_mb_per_s - metrics_alt.throughput_mb_per_s;
  double throughput_ratio =
      metrics_map.throughput_mb_per_s / metrics_alt.throughput_mb_per_s;

  // Print comparison table
  printf("%-25s | %-15.6f | %-15.6f | %-+.2f (%.2fx)\n", "Wall time (s)",
         metrics_map.wall_time, metrics_alt.wall_time, time_diff, time_ratio);

  printf("%-25s | %-15.6f | %-15.6f | %-+.2f\n", "CPU time (s)",
         metrics_map.user_cpu_time + metrics_map.sys_cpu_time,
